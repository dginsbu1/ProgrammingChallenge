...
/******************************************************************************
 * NOT MY WORK. TAKEN FROM https://piazza.com/yu/spring2018/com1320/resources
 ...
 * contains, size, and is-empty</em> methods. A symbol
...
public class BTree<Key extends Comparable<Key>, Value>
{
    ...
    public static class Entry
    {
        ...
        private List<ArrayList<Object>> vals;//@@@
        private Node child;

        public Entry(Comparable key, ArrayList<Object> val, Node child)
        {

            this.key = key;
            this.vals = new ArrayList<ArrayList<Object>>();
            if(val != null) {//don't index null values
                this.vals.add(val);
            }
            this.child = child;
        }
        ...
        public List<ArrayList<Object>> getValue(){
            return this.vals;
        }
     ...
     */
    public List<ArrayList<Object>> get(Key key)//@@@
    {
        if (key == null)
        {
    ...
    public void delete(Key key)//@@@
    {
        ...
        Entry entryToDelete = this.get(this.root, key, this.height);
        if(entryToDelete != null){
            entryToDelete.vals.clear();
        }
        //put(key, null);
    }
    //delete a given row from the BTree
    public void delete(Key key, ArrayList<Object> row){
        if (key == null) {
            throw new IllegalArgumentException("argument key to delete() is null");
        }

        Entry entryToDelete = this.get(this.root, key, this.height);
        if(entryToDelete != null && entryToDelete.vals != null){
            if(entryToDelete.vals.contains(row)){
                entryToDelete.vals.remove(row);
            }
        }
    }

    /**
     * Inserts the key-value pair into the symbol table, adding the old
     * value to the arrayList with the new value if the key is already in the symbol table.
     * value is never null
     * symbol table.
     *
     * @param key the key
    ...
    public void put(Key key, Value val)
    {
        ...
        //if the key already exists in the b-tree, add the value to the List of rows
        Entry alreadyThere = this.get(this.root, key, this.height);
        if(alreadyThere != null)
        {
            alreadyThere.vals.add((ArrayList<Object>)(val));
            return;
        }
    ...
    {
        int j;
        Entry newEntry = new Entry(key, (ArrayList<Object>)val, null);
